...
SoxClient.prototype.unsubscribeDevice = function(device) {
	...
	var successDataCallback = function(data) {
		...
		this.connection.PubSub.unsubscribe(device.nodeName + "_meta", this.jid, device.metaSubid).done(successMetaCallback).fail(failureCallback);
	};
	var failureCallback = function(data) {
		var nodeName = $(data).find('subscription').attr('node');
	...
	this.connection.PubSub.unsubscribe(device.nodeName + "_data", this.jid, device.dataSubid).done(successDataCallback).fail(failureCallback);
